...
def test_list_tools(env):
    ...
    expected = [
        ...
        "request_project_extension", "pass_on_project", "_override_cash", "_override_satisfaction"
    ]
...
def test_get_client_state_specific(env):
    env.reset(seed=3)
    clients = step(env, "get_client_state").result["tool_result"]["clients"]
...
def test_interview_candidate(env):
    env.reset(seed=11)
    cands = step(env, "find_candidate").result["tool_result"]["candidates"]
...
def test_interview_moves_candidate_to_pipeline(env):
    ...
    cstate = step(env, "get_candidate_state").result["tool_result"]
...
def test_hire_deducts_cash(env):
    ...
    cid = cands[0]["id"]
    step(env, "interview_candidate", candidate_id=cid)
    cash_before = step(env, "get_financial_summary").result["tool_result"]["cash_balance"]
    step(env, "hire_candidate", candidate_id=cid)
    cash_after = step(env, "get_financial_summary").result["tool_result"]["cash_balance"]
...
def test_confirm_project(env):
    ...
    projects = step(env, "find_available_projects").result["tool_result"]["projects"]
    if not projects:
...
def test_full_hire_and_place_pipeline(env):
    ...
    cands = step(env, "find_candidate", developer_type=needed_type).result["tool_result"]["candidates"]
    ...
    # Pipeline
    step(env, "interview_candidate", candidate_id=cid)
    hire_r = step(env, "hire_candidate", candidate_id=cid).result["tool_result"]
...
def test_let_go_candidate(env):
    ...
    profile = step(env, "get_candidate_profile", candidate_id=cid).result["tool_result"]
...
def test_pass_on_project(env):
    ...
    remaining = step(env, "find_available_projects").result["tool_result"]["projects"]
...
def test_unknown_tool_returns_error(env):
    ...
    obs = env.step(action)
    result = obs.result.get("tool_result", {})
    # Should return an error, not crash
    assert "error" in result or "tool_result" in obs.result
